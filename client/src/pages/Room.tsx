  ...
  useEffect(() => {
    ...
    api.post(`/rooms/${code}/join`)
      .then(({ data }) => {
        ...
        // Capture session id for feedback redirect
        const mySession = data.room.sessions?.find((s: any) => s.userId === user.id);
        if (mySession) setSessionId(mySession.id);
    ...
    <div className="h-screen bg-zinc-950 text-white flex flex-col overflow-hidden">
      ...
      <nav className="shrink-0 border-b border-zinc-800 px-5 py-2.5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          {isInterviewer && (
            <EndSessionButton roomCode={code!} sessionId={sessionId} />
          )}
        </div>
      </nav>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="text-xs px-4 py-1.5 bg-violet-600 hover:bg-violet-500
                           disabled:opacity-50 disabled:cursor-not-allowed rounded font-medium
